...
!function(window, document, Math) {
    ...
    IScroll.prototype = {
        ...
        _move: function(e) {
            if (this.enabled && utils.eventType[e.type] === this.initiated) {
                ...
                !(timestamp - this.endTime > 300 && 10 > absDistX && 10 > absDistY)) {
                    if (this.directionLocked || this.options.freeScroll || (absDistX > absDistY + this.options.directionLockThreshold ? this.directionLocked = "h" : absDistY >= absDistX + this.options.directionLockThreshold ? this.directionLocked = "v" : this.directionLocked = "n"), 
        ...
        _translate: function(x, y) {
           ...
           // block footer
            /*$("#productinfo").attr("ys", y);
            $('.content_footer div').removeClass('btn').addClass('btntrans');
            if ($('#productinfo').attr("ys") < -4400){
                $('.content_footer div').addClass('btn').removeClass('btntrans');
            };*/
            //end block footer
        },
        ...
        _animate: function(destX, destY, duration, easingFn) {
            function step() {
                ...
                that._translate(newX, newY), void (that.isAnimating && rAF(step)));
            }
        ...
        _move: function(e) {
            ...
            newX = this.x + deltaX, newY = this.y + deltaY, this._pos(newX, newY), e.preventDefault(), 
            e.stopPropagation();
        },
        _end: function(e) {
            if (this.initiated) {
